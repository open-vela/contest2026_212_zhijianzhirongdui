<template>
  <n-layout class="admin-layout" has-sider>
    ...
    <n-layout-sider
      ...
      :class="{ 'ios-glass': theme === 'glass', 'hyperos': theme === 'hyperos' }"
      bordered
    >
      <div class="sidebar-header" @click="collapsed = !collapsed">
        <div v-if="!collapsed" class="logo-text">🏢 枢络</div>
    ...
    <n-layout>
      <!-- 顶部导航 -->
      <n-layout-header class="admin-header" :class="{ 'ios-glass': theme === 'glass' }" bordered>
...
// 菜单定义（对齐管理端 9 模块: 仪表板/人员/访客/审计/设备/空间/能源/设置）
